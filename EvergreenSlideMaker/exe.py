...
hymn_list = ['주님은 나의 힘이요', '나의 가장 낮은 마음', '갈보리산 위에']
...
def create_presentation(hymn_list=[]):
    ...
    prs.slide_width = Cm(33.867)
    prs.slide_height = Cm(19.05)
    ['주님은 나의 힘이요', '나의 가장 낮은 마음', '갈보리산 위에']
    directory = folder_path+"/bible"
    pic_dic = folder_path+"/image/"
    
    
    add_image_slide(prs, pic_dic+'2025.jpg', text='송구영신예배')
    add_blank_slide(prs)
    add_hymn_slide(prs, hymn_list[0])
    add_hymn_slide(prs, hymn_list[1])
    add_hymn_slide(prs, hymn_list[2])
    ...
    add_card_slide(prs, input_text= '성찬')
    add_hymn_slide(prs, '내 주의 보혈은')   
    add_blank_slide(prs)
 

    # 제목/본문 (필수)
    add_subtitle_slide(prs, input_text="믿음으로 달려가는 한 해 (히브리서 12:1)")
    
    # RED only — 원고 등장 순서
    add_bible_slide(prs, directory, "히브리서", "10:32", "10:33")
    add_bible_slide(prs, directory, "히브리서", "10:34")
    add_bible_slide(prs, directory, "히브리서", "10:23")
    add_bible_slide(prs, directory, "히브리서", "10:35")
    add_bible_slide(prs, directory, "히브리서", "10:36")
    
    add_bible_slide(prs, directory, "시편", "103:12")
    
    add_bible_slide(prs, directory, "히브리서", "12:2")
    add_bible_slide(prs, directory, "히브리서", "12:3")
    
    add_bible_slide(prs, directory, "시편", "23:1")
    add_bible_slide(prs, directory, "이사야", "41:10")
    add_bible_slide(prs, directory, "히브리서", "13:5")
    
    add_bible_slide(prs, directory, "고린도후서", "5:7")
    add_bible_slide(prs, directory, "마태복음", "26:39")
    add_bible_slide(prs, directory, "누가복음", "22:31", "22:32")
    add_bible_slide(prs, directory, "잠언", "24:16")
    add_bible_slide(prs, directory, "빌립보서", "1:6")

   
    add_card_slide(prs, input_text= '회개와 결단의 기도', background_color='000000')
    add_blank_slide(prs)
    add_card_slide(prs, input_text= '침묵기도', background_color='000000')

    add_hymn_slide(prs, '시온의 영광이 빛나는 아침')
    add_card_slide(prs, input_text= '말씀카드')
    add_card_slide(prs, input_text= '통성기도', background_color='000000')
    add_card_slide(prs, input_text= '신유축복기도')
    add_card_slide(prs, input_text= '신앙선언')
